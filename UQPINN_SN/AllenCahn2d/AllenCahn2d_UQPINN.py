...
def get_r (model_P,inp_t_Nf, inp_x_Nf, inp_y_Nf, inp_z_Nf,device,Jacobian_X,Jacobian_Y,Jacobian_T):
    u = model_P(torch.cat([inp_t_Nf, inp_x_Nf,inp_y_Nf, inp_z_Nf], dim=1))
    ...
    u_y = autograd.grad(outputs=u, inputs=inp_y_Nf,
                        grad_outputs=torch.ones(u.size()).to(device),
                        create_graph=True,
                        )[0]

    u_yy = autograd.grad(outputs=u_y, inputs=inp_y_Nf,
    ...
    r = Jacobian_T*u_t + torch.pow(u, 3) - u - Jacobian_X*Jacobian_X*u_xx - Jacobian_Y*Jacobian_Y*u_yy -(2 * torch.exp(-inp_t_Nf) * torch.sin(math.pi * inp_x_Nf) * torch.sin(math.pi * inp_y_Nf) * (
                                             math.pi * math.pi - 1) + torch.exp(-3 * inp_t_Nf) * torch.pow(torch.sin(math.pi * inp_x_Nf), 3) * torch.pow(torch.sin(math.pi * inp_y_Nf), 3))
...
def train(model_P,model_Q,model_T,Nf_dataset,Nu_dataset,loss_fn,device,num_epochs,optimizer_G,optimizer_T,text_x,text_t,text_y,text_z,lam = 1.5, beta = 1.,k1=1,k2=5):
    ...
    for epoch_idx in range(num_epochs):
        ...
        for batch_Nf,batch_Nu in zip(data_loader_Nf,data_loader_Nu):
            ...
            value_Nu_pred = model_P (torch.cat([inp_t_Nu,inp_x_Nu,inp_y_Nu,inp_z_Nu], dim = 1)).to(device)
            Residual = get_r(model_P,inp_t_Nf, inp_x_Nf, inp_y_Nf, inp_z_Nf,device,Jacobian_X,Jacobian_Y,Jacobian_T)

            G_loss, KL_loss, recon_loss, PDE_loss=compute_generator_loss(model_P,model_Q,model_T,inp_x_Nu,inp_y_Nu,inp_t_Nu,value_Nu,value_Nu_pred,inp_x_Nf,inp_y_Nf,inp_t_Nf,
                                                                         Residual,inp_z_Nu,inp_z_Nf,lam,beta)

            T_loss = compute_discriminator_loss(model_P,model_Q,model_T,inp_x_Nu,inp_y_Nu , inp_t_Nu, value_Nu, inp_z_Nu)
    ...
    with torch.no_grad():
        ...
        u_text = model_P(torch.cat([inp_t_text, inp_x_text, inp_y_text,inp_z_text], dim=1)).to(device)
...
def AllenCahn2d_UQPINN(Nf_dataset, Nu_dataset,text_x,text_t,text_y,num_epochs=100):
    ...
    if args.optimizer =='Adam':
        optimizer_G = torch.optim.Adam(   [{'params':model_P.parameters(),'lr':1e-5},
                                        {'params': model_Q.parameters(), 'lr': 1e-5},
                                        ],)
    else:
        optimizer_G = torch.optim.SGD([
            {'params':model_P.parameters(),'lr':1e-4},
            {'params': model_Q.parameters(), 'lr': 1e-4},], lr=1e-2, momentum=0.9)

    if args.optimizer =='Adam':
        optimizer_T = torch.optim.Adam(   [{'params': model_T.parameters(), 'lr': 1e-5},],)
    else:
