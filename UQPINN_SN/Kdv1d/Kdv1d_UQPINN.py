...
#         self.middle_layer = nn.Linear(hidden_dim, hidden_dim,bias=True)
...
def get_r (model_P,inp_t_Nf, inp_x_Nf, inp_z_Nf, device,Jacobian_X,Jacobian_T):
    u = model_P(torch.cat([inp_t_Nf, inp_x_Nf, inp_z_Nf], dim=1))
    ...
    u_xxx = autograd.grad(outputs=u_xx, inputs=inp_x_Nf,
                        grad_outputs=torch.ones(u_xx.size()).to(device),
                        create_graph=True,
                         )[0]
    ...
    r = Jacobian_T*u_t + Jacobian_X*(math.pow(g * h, 1 / 2) + 3 * math.pow(g * h, 1 / 2) * u / (2 * h)) * u_x +Jacobian_X*Jacobian_X*Jacobian_X* (
                h * h * math.pow(g * h, 1 / 2) / 6) * u_xxx

    # r = Jacobian_T*u_t+Jacobian_X*u*u_x-(0.01/math.pi)*Jacobian_X*Jacobian_X*u_xx

    return r

def compute_discriminator_loss(model_P,model_Q,model_T,inp_x_Nu, inp_t_Nu, value_Nu, inp_z_Nu):

    value_Nu_pred = model_P(torch.cat([inp_t_Nu, inp_x_Nu, inp_z_Nu], dim = 1))
...
def compute_generator_loss(model_P,model_Q,model_T,inp_x_Nu,inp_t_Nu,value_Nu,value_Nu_pred,inp_x_Nf,inp_t_Nf,Residual,inp_z_Nu,inp_z_Nf,lam,beta):
    ...
    value_pred = model_P(torch.cat([inp_t_Nu,inp_x_Nu,inp_z_Nu], dim = 1))
...
def train(model_P,model_Q,model_T,Nf_dataset,Nu_dataset,loss_fn,device,num_epochs,optimizer_G,optimizer_T,text_x,text_t,text_y,text_z,lam = 1.5, beta = 1.,k1=1,k2=5):
    ...
    for epoch_idx in range(num_epochs):
        ...
        for batch_Nf,batch_Nu in zip(data_loader_Nf,data_loader_Nu):
            ...
            value_Nu_pred = model_P(torch.cat([inp_t_Nu,inp_x_Nu,inp_z_Nu],dim=1))
            Residual = get_r(model_P,inp_t_Nf, inp_x_Nf, inp_z_Nf,device,Jacobian_X,Jacobian_T)

            G_loss, KL_loss, recon_loss, PDE_loss=compute_generator_loss(model_P,model_Q,model_T,inp_x_Nu,inp_t_Nu,value_Nu,value_Nu_pred,inp_x_Nf,inp_t_Nf,
                                                                         Residual,inp_z_Nu,inp_z_Nf,lam,beta)

            T_loss = compute_discriminator_loss(model_P,model_Q,model_T,inp_x_Nu, inp_t_Nu, value_Nu, inp_z_Nu)
            ...
            for i in range(k2):
                ...
                optimizer_G.step()
            # print('KL_loss: %.2e, Recon_loss: %.2e, PDE_loss: %.2e, T_loss: %.2e' %
            #       (KL_loss, recon_loss, PDE_loss, T_loss))
    with torch.no_grad():
        Inference_time = time.time()
        # inp_z_text = torch.randn(inp_x_text.size()[0]).unsqueeze(-1).to(device)
        u_text = model_P(torch.cat([inp_t_text, inp_x_text,inp_z_text], dim=1)).to(device)
        Inference_time = time.time() - Inference_time
        loss_text = torch.sum(torch.pow(u_text - value_text, 2)) / u_text.size()[0]
        print(['training_loss', PDE_loss, 'text_loss:{:.16f}'.format(loss_text)])
...
def Kdv1d_UQPINN(Nf_dataset, Nu_dataset,text_x,text_t,text_y,num_epochs=150):
