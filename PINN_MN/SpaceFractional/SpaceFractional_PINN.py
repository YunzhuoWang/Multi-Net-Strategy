...
class AVEMSE(nn.Module):
    ...
    def forward(self, x,t , output_Nu,Nu,u,u_t,u_alpha,gamma,alpha,u_t_grad):
        ...
        loss_adjustment = torch.sum(torch.pow(u_t-u_t_grad,2)/u_t.size()[0])
        loss = loss_b+loss_f+loss_adjustment
        return loss
...
def train(model_0gradient,model_2gradient,model_tgradient,Nf_dataset,Nu_dataset,loss_fn,device,num_epochs,optimizer,text_x,text_t,text_y,alpha,gamma):
    ...
    for epoch_idx in range(num_epochs):
        ...
        for batch_Nf,batch_Nu in zip(data_loader_Nf,data_loader_Nu):
            ...
            u_alpha = model_2gradient(torch.cat([inp_t_Nf, inp_x_Nf], dim=1)).to(device)
            ...
            loss = loss_fn(inp_x_Nf,inp_t_Nf,output_Nu,value_Nu,u,u_t,u_alpha,alpha,gamma,u_t_grad)
...
def SpaceFractional_PINN(Nf_dataset, Nu_dataset,text_x,text_t,text_y,num_epochs=100):
    ...
    model_0gradient = NN(50,2,1,15)
    model_2gradient = NN(50, 2, 1, 15)
    model_tgradient = NN(50, 2, 1, 15)
