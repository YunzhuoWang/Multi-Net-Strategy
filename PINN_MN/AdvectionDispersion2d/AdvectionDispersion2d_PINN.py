...
class AVEMSE(nn.Module):
    ...
    def forward(self, output_Nu,Nu,u,u_t,u_x,u_xx,u_y,u_yy,u_t_grad):
        ...
        loss_adjustment = torch.sum(torch.pow(u_t-u_t_grad,2))
...
def train(model_0gradient,model_1gradient,model_2gradient,model_tgradient,Nf_dataset,Nu_dataset,loss_fn,device,num_epochs,optimizer,text_x,text_t,text_y):
    ...
    for epoch_idx in range(num_epochs):
        ...
        for batch_Nf,batch_Nu in zip(data_loader_Nf,data_loader_Nu):
            ...
            u_x = model_1gradient (torch.cat([inp_t_Nf,inp_x_Nf,inp_y_Nf], dim = 1))[:,0].to(device)

            u_y =  model_1gradient (torch.cat([inp_t_Nf,inp_x_Nf,inp_y_Nf], dim = 1))[:,1].to(device)
            ...
            loss = loss_fn(output_Nu,value_Nu,u,u_t,u_x,u_xx,u_y,u_yy,u_t_grad)
