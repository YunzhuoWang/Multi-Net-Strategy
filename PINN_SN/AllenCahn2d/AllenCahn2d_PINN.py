...
class AVEMSE(nn.Module):
    ...
    def forward(self,t,x,y,output_Nu,Nu,u,u_t,u_x,u_xx,u_y,u_yy):
        loss_b = torch.sum(torch.pow(output_Nu-Nu,2))/output_Nu.size()[0]
        loss_f = torch.sum(torch.pow(u_t+torch.pow(u,3)-u-u_xx-u_yy-
                                     (2*torch.exp(-t)*torch.sin(math.pi*x)*torch.sin(math.pi*y)*(math.pi*math.pi-1)+torch.exp(-3*t)*torch.pow(torch.sin(math.pi*x),3)*torch.pow(torch.sin(math.pi*y),3)),2))/u_t.size()[0]
        loss = loss_b+loss_f
        return loss
...
def train(model,Nf_dataset,Nu_dataset,loss_fn,device,num_epochs,optimizer,text_x,text_t,text_y):
    ...
    for epoch_idx in range(num_epochs):
        ...
        for batch_Nf,batch_Nu in zip(data_loader_Nf,data_loader_Nu):
            ...
            u = model (torch.cat([inp_t_Nf,inp_x_Nf,inp_y_Nf], dim = 1)).to(device)
            output_Nu = model (torch.cat([inp_t_Nu,inp_x_Nu,inp_y_Nu], dim = 1)).to(device)
            ...
            loss = loss_fn(inp_t_Nf,inp_x_Nf,inp_y_Nf,output_Nu,value_Nu,u,u_t,u_x,u_xx,u_y,u_yy)
            model.zero_grad()
            loss.backward()
            optimizer.step()
    with torch.no_grad():
        Inference_time = time.time()
        u_text = model(torch.cat([inp_t_text, inp_x_text,inp_y_text], dim=1)).to(device)
