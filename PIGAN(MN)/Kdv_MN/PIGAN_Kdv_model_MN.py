# File      :PIGAN_Kdv_model_MN.py
# Time      :2021/5/21--20:11
# Author    :JF Li
# Version   :python 3.7
...
def train(generator, generator_u_t, generator_u_x, generator_u_xxx, discriminator, g_optimizer, d_optimizer, device,
    ...
    for itr in range(num_iter):
        ...
        for batch_Nf, batch_Nu in zip(dataloader_Nf, dataloader_Nu):
            ...
            train_point_interior = Variable(batch_Nf, requires_grad = True).to(device)
            ...
            u_x = generator_u_x(train_point_interior)
            u_xxx = generator_u_xxx(train_point_interior)
            # 方程残差
            residual = u_t + (np.sqrt(9.8 * 6) + 3 * np.sqrt(9.8 * 6) / (2 * 6) * train_point_interior_pred) * u_x + (
                    6 * 6 * np.sqrt(9.8 * 6)) / 6 * u_xxx
            # 判别器的预测值
...
def Kdv_MN(Nf_dataset, Nu_dataset, test_point, test_real_value):
    # 初始设置
    ...
    # 模型定义
    generator = NN(20, 2, 1, 8)  # 生成器
    generator_u_t = NN(10, 2, 1, 3)  # u_t
    generator_u_x = NN(10, 2, 1, 3)  # u_x
    generator_u_xxx = NN(10, 2, 1, 3)  # u_xxx
    discriminator = NN(20, 1, 1, 8)  # 判别器
    generator.to(device)
    generator_u_t.to(device)
    generator_u_x.to(device)
    generator_u_xxx.to(device)
    discriminator.to(device)
    # 优化器设置
    if args.optimizer == 'Adam':
        g_optimizer = torch.optim.Adam([{'params': generator.parameters(), 'lr': 1e-5},
                                        {'params': generator_u_t.parameters(), 'lr': 1e-5},
                                        {'params': generator_u_x.parameters(), 'lr': 1e-5},
                                        {'params': generator_u_xxx.parameters(), 'lr': 1e-5}], lr = 1e-5)
        ...
        g_optimizer = torch.optim.SGD([{'params': generator.parameters(), 'lr': 1e-4},
                                       ...
                                       {'params': generator_u_x.parameters(), 'lr': 1e-4},
                                       {'params': generator_u_xxx.parameters(), 'lr': 1e-4}], lr = 0.001,
                                      momentum = 0.9)
    ...
    loss, times = train(
        ...
        generator_u_x = generator_u_x,
        generator_u_xxx = generator_u_xxx,
