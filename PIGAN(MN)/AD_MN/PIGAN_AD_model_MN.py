# File      :PIGAN_AD_model_MN.py
# Time      :2021/5/21--18:51
# Author    :JF Li
# Version   :python 3.7
...
def train(generator, generator_u_t, generator_u_1, generator_u_2, discriminator,
          g_optimizer, d_optimizer, device, num_iter, Nf_dataset, Nu_dataset, test_point, test_real_value,
          discriminator_iteration, generator_iteration):
    # 处理测试点
    ...
    for itr in range(num_iter):
        ...
        for batch_Nf, batch_Nu in zip(dataloader_Nf, dataloader_Nu):
            ...
            train_point_interior = torch.cat([train_point_interior_t, train_point_interior_x, train_point_interior_y],
                                             dim = 1)
            ...
            u_1 = generator_u_1(train_point_interior)
            u_2 = generator_u_2(train_point_interior)
            # 方程残差
            u_x, u_y = u_1[:, 0].unsqueeze(-1), u_1[:, 1].unsqueeze(-1)
            ...
            G_loss_adjustment = torch.mean(torch.pow(u_t - Du_t, 2))
...
def AD_MN(Nf_dataset, Nu_dataset, test_point, test_real_value):
    # 初始设置
    ...
    if args.optimizer == 'Adam':
        g_optimizer = torch.optim.Adam([{'params': generator.parameters(), 'lr': 1e-4},
                                        ...
                                        {'params': generator_u_1.parameters(), 'lr': 1e-4},
                                        {'params': generator_u_2.parameters(), 'lr': 1e-4}], lr = 1e-4)
        d_optimizer = torch.optim.Adam(discriminator.parameters(), lr = 1e-4)
    else:
        g_optimizer = torch.optim.SGD([{'params': generator.parameters(), 'lr': 1e-5},
                                       {'params': generator_u_t.parameters(), 'lr': 1e-5},
                                       {'params': generator_u_1.parameters(), 'lr': 1e-5},
                                       {'params': generator_u_2.parameters(), 'lr': 1e-5}], lr = 0.001, momentum = 0.9)
        d_optimizer = torch.optim.SGD(discriminator.parameters(), lr = 0.001, momentum = 0.9)
    # 模型训练
    loss, times = train(
        generator = generator,
        generator_u_t = generator_u_t,
        generator_u_1 = generator_u_1,
