# File      :PIGAN_SpaceFrac_model.py
# Time      :2021/5/21--21:27
# Author    :JF Li
# Version   :python 3.7
...
def train(generator, generator_u_t, generator_u_alphax, alpha, discriminator, g_optimizer, d_optimizer, device,
          num_iter, Nf_dataset, Nu_dataset, test_point, test_real_value, discriminator_iteration, generator_iteration):
    # 处理测试点
    ...
    for itr in range(num_iter):
        ...
        for batch_Nf, batch_Nu in zip(dataloader_Nf, dataloader_Nu):
            ...
            train_point_interior = torch.cat([train_point_interior_t, train_point_interior_x], dim = 1)

            # 处理边界集
            train_point_boundary = Variable(batch_Nu[:, 0:2], requires_grad = True).to(device)
            train_value_boundary = Variable(batch_Nu[:, 2], requires_grad = True).unsqueeze(-1).to(device)

            # 计算生成器预测值
            train_point_boundary_pred = generator(train_point_boundary).to(device)
            train_point_interior_pred = generator(train_point_interior).to(device)

            # 对非抽样点的内部点生成结果进行后处理(post-processing)
            Du_t = autograd.grad(outputs = train_point_interior_pred, inputs = train_point_interior_t,
                                 grad_outputs = torch.ones(train_point_interior_pred.size()).to(device),
                                 create_graph = True,
                                 )[0]
            u_t = generator_u_t(train_point_interior)
            u_alphax = generator_u_alphax(train_point_interior)
            # 方程残差
            residual = u_t - torch.pow(train_point_interior_x, 2) * torch.sin(-train_point_interior_t) + torch.pow(
                train_point_interior_x, 2) * torch.cos(-train_point_interior_t)
            residual = residual - torch.pow(train_point_interior_x, alpha) * gamma(3 - alpha) / 2 * u_alphax
            # 判别器的预测值
            ...
            G_loss_adjustment = torch.mean(torch.pow(Du_t - u_t, 2))
            G_total_loss = G_loss_boundary + G_loss_interior + G_loss_adjustment
            # 计算判别器误差
...
def SpaceFrac_MN(Nf_dataset, Nu_dataset, test_point, test_real_value):
    # 初始设置
    ...
    parser.add_argument('--generator_iteration', type = int, default = 2)
    ...
    # 模型定义
    generator = NN(50, 2, 1, 10)  # 生成器
    generator_u_t = NN(20, 2, 1, 5)  # u_t
    generator_u_alphax = NN(20, 2, 1, 5)  # u_αx
    discriminator = NN(50, 1, 1, 8)  # 判别器
    generator.to(device)
    generator_u_t.to(device)
    generator_u_alphax.to(device)
    discriminator.to(device)
    # 优化器设置
    if args.optimizer == 'Adam':
        g_optimizer = torch.optim.Adam([{'params': generator.parameters(), 'lr': 1e-4},
                                        {'params': generator_u_t.parameters(), 'lr': 1e-4},
                                        {'params': generator_u_alphax.parameters(), 'lr': 1e-4}], lr = 1e-4)
        d_optimizer = torch.optim.Adam(discriminator.parameters(), lr = 1e-4)
    else:
        g_optimizer = torch.optim.SGD([{'params': generator.parameters(), 'lr': 1e-4},
                                       {'params': generator_u_t.parameters(), 'lr': 1e-4},
                                       {'params': generator_u_alphax.parameters(), 'lr': 1e-4}], lr = 0.001,
                                      momentum = 0.9)
        d_optimizer = torch.optim.SGD(discriminator.parameters(), lr = 0.001, momentum = 0.9)
    # 模型训练
    loss, times = train(
        generator = generator,
        generator_u_t = generator_u_t,
        generator_u_alphax = generator_u_alphax,
        alpha = args.alpha,
