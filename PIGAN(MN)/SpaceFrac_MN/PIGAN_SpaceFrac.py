# File      :PIGAN_SpaceFrac.py
# Time      :2021/5/21--21:26
# Author    :JF Li
# Version   :python 3.7
...
import scipy.io
import torch
from PIGAN_SpaceFrac_model import SpaceFrac_MN


def main():
    # 读入测试集
    np.random.seed(42)
    test = scipy.io.loadmat('./Data/SpaceFractional_exact.mat')
    test_point_t = test['t']
    test_point_x = test['coordinate']
    ...
    for i in range(run_times):
        PIGAN_error, PIGAN_time = SpaceFrac_MN(Nf_dataset, Nu_dataset, test_point, test_real_value)
        MSE += PIGAN_error['MSE']
