# File      :PIGAN_Burgers1d_model_MN.py
# Time      :2021/5/17--14:31
# Author    :JF Li
# Version   :python 3.7
...
def train(generator, generator_u_t, generator_u_x, generator_u_xx, discriminator, g_optimizer, d_optimizer, device,
    ...
    for itr in range(num_iter):
        dataloader_Nf = DataLoader(Nf_dataset, batch_size = 14)  # , shuffle = True
        ...
        for batch_Nf, batch_Nu in zip(dataloader_Nf, dataloader_Nu):
            ...
            u_xx = generator_u_xx(train_point_interior)
            # 方程残差
            residual = u_t + train_point_interior_pred * u_x - (0.01 / np.pi) * u_xx
            # 判别器的预测值
...
def Burgers_1d_MN(Nf_dataset, Nu_dataset, test_point, test_real_value):
    # 初始设置
    ...
    if args.optimizer == 'Adam':
        g_optimizer = torch.optim.Adam([{'params': generator.parameters(), 'lr': 1e-4},
                                        ...
                                        {'params': generator_u_xx.parameters(), 'lr': 1e-4}], lr = 1e-4)
        d_optimizer = torch.optim.Adam(discriminator.parameters(), lr = 1e-4)
    else:
        g_optimizer = torch.optim.SGD([{'params': generator.parameters(), 'lr': 1e-4},
                                       {'params': generator_u_t.parameters(), 'lr': 1e-4},
                                       {'params': generator_u_x.parameters(), 'lr': 1e-4},
                                       {'params': generator_u_xx.parameters(), 'lr': 1e-4}], lr = 0.001,
                                      momentum = 0.9)
