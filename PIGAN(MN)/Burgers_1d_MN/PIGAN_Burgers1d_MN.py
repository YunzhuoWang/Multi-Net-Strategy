# File      :PIGAN_Burgers1d_MN.py
# Time      :2021/5/17--14:30
# Author    :JF Li
# Version   :python 3.7
...
import scipy.io
import torch
from PIGAN_Burgers1d_model_MN import Burgers_1d_MN


def main():
    # 读入测试集
    np.random.seed(42)
    test = scipy.io.loadmat('./Data/burgers_shock.mat')
    test_point_t = test['t']
    test_point_x = test['x']
    test_real_value = test['usol']

    test_point_x, test_point_t = np.meshgrid(test_point_x, test_point_t)
    test_point_x = test_point_x.flatten()[:, None]
    test_point_t = test_point_t.flatten()[:, None]
    test_real_value = test_real_value.T
    ...
    test_point = torch.cat((test_point_t, test_point_x), dim = 1)
    ...
    for i in range(run_times):
        PIGAN_error, PIGAN_time = Burgers_1d_MN(Nf_dataset, Nu_dataset, test_point, test_real_value)
        MSE += PIGAN_error['MSE']
