# File      :PIGAN_AllenCahn_model_MN.py
# Time      :2021/5/21--21:01
# Author    :JF Li
# Version   :python 3.7
...
def train(generator, generator_u_t, generator_u_2, discriminator, g_optimizer, d_optimizer, device,
    ...
    for itr in range(num_iter):
        ...
        for batch_Nf, batch_Nu in zip(dataloader_Nf, dataloader_Nu):
            ...
            u_2 = generator_u_2(train_point_interior)
            # 方程残差
            u_xx, u_yy = u_2[:, 0].unsqueeze(-1), u_2[:, 1].unsqueeze(-1)
            f = 2 * torch.exp(-train_point_interior[:, 0].unsqueeze(-1)) * torch.sin(
                np.pi * train_point_interior[:, 1].unsqueeze(-1)) * torch.sin(
                np.pi * train_point_interior[:, 2].unsqueeze(-1)) * (np.pi * np.pi - 1) + torch.exp(
                -3 * train_point_interior[:, 0].unsqueeze(-1)) * torch.pow(
                torch.sin(np.pi * train_point_interior[:, 1].unsqueeze(-1)), 3) * torch.pow(
                torch.sin(np.pi * train_point_interior[:, 2].unsqueeze(-1)), 3)
            residual = u_t + torch.pow(train_point_interior_pred, 3) - train_point_interior_pred - (u_xx + u_yy) - f
            # 判别器的预测值
...
def AllenCahn_MN(Nf_dataset, Nu_dataset, test_point, test_real_value):
    # 初始设置
