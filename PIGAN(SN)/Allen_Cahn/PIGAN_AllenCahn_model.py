# File      :PIGAN_AllenCahn_model.py
# Time      :2021/5/21--20:33
# Author    :JF Li
# Version   :python 3.7
...
def train(generator, discriminator, g_optimizer, d_optimizer, device, num_iter, Nf_dataset, Nu_dataset, test_point,
    ...
    for itr in range(num_iter):
        ...
        for batch_Nf, batch_Nu in zip(dataloader_Nf, dataloader_Nu):
            ...
            # 方程残差
            f = 2 * torch.exp(-train_point_interior_t) * torch.sin(np.pi * train_point_interior_x) * torch.sin(
                np.pi * train_point_interior_y) * (np.pi * np.pi - 1) + torch.exp(
                -3 * train_point_interior_t) * torch.pow(torch.sin(np.pi * train_point_interior_x), 3) * torch.pow(
                torch.sin(np.pi * train_point_interior_y), 3)
...
def AllenCahn(Nf_dataset, Nu_dataset, test_point, test_real_value):
    # 初始设置
