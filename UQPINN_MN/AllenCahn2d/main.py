...
import scipy.io
import csv
from AllenCahn2d_UQPINN import AllenCahn2d_UQPINN

import pandas as pd
def main():
    #读入测试集
    np.random.seed(42)
    text = scipy.io.loadmat('./data/AllenCahn_exact.mat')
    ...
    run_times=1
    _AllenCahn2d_error_evermoment=[[i*0.01,0.0] for i in range(100)]
    _Inference_time=0
    _AllenCahn2d_error=0
    _Training_time=0
    ...
    for i in range(run_times):
        AllenCahn2d_start_time = time.time()
        AllenCahn2d_error_evermoment, AllenCahn2d_error,Inference_time, value_4, value_8= AllenCahn2d_UQPINN(Nf_dataset, Nu_dataset,text_x,text_t,text_y)
        AllenCahn2d_time = time.time()-AllenCahn2d_start_time
        ...
        _AllenCahn2d_error_evermoment = [[_AllenCahn2d_error_evermoment[i][0],_AllenCahn2d_error_evermoment[i][1]+AllenCahn2d_error_evermoment[i][1]] for i in range(100)]
        _Inference_time += Inference_time
        _AllenCahn2d_error += AllenCahn2d_error
        _Training_time += AllenCahn2d_time-Inference_time
        print({'AllenCahn2d_time ': AllenCahn2d_time , 'AllenCahn2d_error':AllenCahn2d_error})
    _AllenCahn2d_error_evermoment = [[_AllenCahn2d_error_evermoment[i][0], _AllenCahn2d_error_evermoment[i][1]/run_times] for i in range(100)]
    ...
    dataframe = pd.DataFrame({'t':[_AllenCahn2d_error_evermoment[i][1]for i in range(100)], 'loss': [_AllenCahn2d_error_evermoment[i][0] for i in range(100)]})
    dataframe.to_csv("./Results/UQPINN_MN_AllenCahn2d_noise10_evermoment_loss.csv", index=False,header=False, sep=',')
    ...
    # dataframe.to_csv("./Results/UQPINN_MN_AllenCahn2d_noise10_value_4_approximate.csv", index=False, header=False, sep=',')
    #
    ...
    # dataframe.to_csv("./Results/UQPINN_MN_AllenCahn2d_noise10_value_8_approximate.csv", index=False, header=False, sep=',')
    #
    ...
    # dataframe.to_csv("./Results/UQPINN_MN_AllenCahn2d_noise10_value_4_exact.csv", index=False, header=False, sep=',')
    #
    ...
    # dataframe.to_csv("./Results/UQPINN_MN_AllenCahn2d_noise10_value_8_exact.csv", index=False, header=False, sep=',')

    with open('./Results/UQPINN_MN_AllenCahn2d_loss_noise10.txt', 'w') as f:  # 设置文件对象
        f.write("loss:{},'Training_time:{},'Inference_time:'{}".format(_AllenCahn2d_error/run_times,_Training_time/run_times,_Inference_time/run_times))
        # print({'AllenCahn2d_time ': AllenCahn2d_time , 'AllenCahn2d_error':AllenCahn2d_error})
