...
def get_r (model_P_tgradient,model_P_0gradient,model_P_2gradient,inp_t_Nf, inp_x_Nf, inp_y_Nf, inp_z_Nf,device,Jacobian_X,Jacobian_Y,Jacobian_T):
    u =  model_P_0gradient(torch.cat([inp_t_Nf, inp_x_Nf, inp_y_Nf, inp_z_Nf], dim=1))[:,0].unsqueeze(-1)
    ...
    r = Jacobian_T * u_t + torch.pow(u, 3) - u - Jacobian_X * Jacobian_X * u_xx - Jacobian_Y * Jacobian_Y * u_yy - (
                2 * torch.exp(-inp_t_Nf) * torch.sin(math.pi * inp_x_Nf) * torch.sin(math.pi * inp_y_Nf) * (
                math.pi * math.pi - 1) + torch.exp(-3 * inp_t_Nf) * torch.pow(torch.sin(math.pi * inp_x_Nf),
                                                                              3) * torch.pow(
            torch.sin(math.pi * inp_y_Nf), 3))
...
def train(model_P_tgradient,model_P_0gradient,model_P_2gradient,model_Q,model_T,Nf_dataset,Nu_dataset,loss_fn,device,num_epochs,optimizer_G,optimizer_T,text_x,text_t,text_y,text_z,lam = 1.5, beta = 1.,k1=1,k2=5):
    ...
    for epoch_idx in range(num_epochs):
        ...
        for batch_Nf,batch_Nu in zip(data_loader_Nf,data_loader_Nu):
            ...
            Residual = get_r(model_P_tgradient,model_P_0gradient,model_P_2gradient,inp_t_Nf, inp_x_Nf, inp_y_Nf, inp_z_Nf,device,Jacobian_X,Jacobian_Y,Jacobian_T)
...
def AllenCahn2d_UQPINN(Nf_dataset, Nu_dataset,text_x,text_t,text_y,num_epochs=100):
