...
def main():
    np.random.seed(42)
    downboundary_x, upboundary_x = -1, 1
    ...
    sio.savemat('./Data/Burgers_training_Nf.mat', {'coordinate': Nf_coordinate})
    #构造Nu训练集
    Nu = 128
    #边界1
    ...
    #边界2
    x_set = -1*np.ones(2*Nu).reshape(2*Nu,1)
    t_set = downboundary_t + (upboundary_t - downboundary_t) * np.random.rand(2*Nu).reshape(2*Nu, 1)
    z_set = np.zeros(2*Nu).reshape(2*Nu,1)
    ...
    sio.savemat('./Data/Burgers_training_Nu.mat', {'coordinate': Nu_coordinate[:,0:2],'value':Nu_coordinate[:,-1].reshape(Nu,1)})
