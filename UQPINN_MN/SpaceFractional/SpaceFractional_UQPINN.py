...
def get_r (model_P_tgradient,model_P_2gradient,inp_t_Nf, inp_x_Nf, inp_z_Nf,device,Jacobian_X,Jacobian_T,alpha,gamma,u_t_grad):

    u_alpha = model_P_2gradient(torch.cat([inp_t_Nf, inp_x_Nf, inp_z_Nf], dim=1))

    u_t = model_P_tgradient(torch.cat([inp_t_Nf, inp_x_Nf, inp_z_Nf], dim=1))
    Jacobian_X = torch.ones(1)*Jacobian_X
    Jacobian_X = Jacobian_X.unsqueeze(-1).to(device)

    r = Jacobian_T*u_t+torch.pow(Jacobian_X,alpha)*gamma*torch.pow(inp_x_Nf,alpha)*u_alpha/2-torch.sin(-inp_t_Nf)*torch.pow(inp_x_Nf,2)+torch.cos(-inp_t_Nf)*torch.pow(inp_x_Nf,2)+torch.pow(u_t_grad-u_t,2)
...
class AVEMSE(nn.Module):
    ...
    def forward(self, x,t , output_Nu,Nu,u,u_t,u_alpha,gamma,alpha):
        loss_b = torch.sum(torch.pow(output_Nu-Nu,2))/output_Nu.size()[0]
        loss_f = torch.sum(torch.pow(u_t-gamma*torch.pow(x,alpha)*u_alpha/2-torch.sin(-t)*torch.pow(x,2)+torch.cos(-t)*torch.pow(x,2),2))/u_t.size()[0]
        loss = loss_b+loss_f
        return loss
...
def train(model_P_tgradient,model_P_0gradient,model_P_2gradient,model_Q,model_T,Nf_dataset,Nu_dataset,loss_fn,device,num_epochs,optimizer_G,optimizer_T,text_x,text_t,text_y,text_z,alpha,gamma,lam,beta,k1,k2):
    inp_z_text = Variable(text_z.flatten(), requires_grad=False).unsqueeze(-1).to(device)
    inp_x_text = Variable(text_x.flatten(), requires_grad=False).unsqueeze(-1).to(device)
    ...
    for epoch_idx in range(num_epochs):
        ...
        for batch_Nf,batch_Nu in zip(data_loader_Nf,data_loader_Nu):
            ...
            value_Nf_pred = model_P_0gradient(torch.cat([inp_t_Nf,inp_x_Nf,inp_z_Nf],dim=1))

            u_t = autograd.grad(outputs=value_Nf_pred, inputs=inp_t_Nf,
                                grad_outputs=torch.ones(value_Nf_pred.size()).to(device),
                                create_graph=True,
                                )[0]

            Residual = get_r(model_P_tgradient,model_P_2gradient,inp_t_Nf, inp_x_Nf, inp_z_Nf,device,Jacobian_X,Jacobian_T,alpha,gamma,u_t)
    ...
    loss_evermoment = []
    for i in range(100):
        loss_evermoment.append([i*0.01,float(torch.sum(loss_[i*5:(i+1)*5])/5)])
    return loss_evermoment,loss_text,Inference_time,u_text[40*5:41*5],u_text[80*5:81*5]

def SpaceFractional_UQPINN(Nf_dataset, Nu_dataset,text_x,text_t,text_y,num_epochs=100):
    ...
    alpha = 1.8
    gamma = math.gamma(3-alpha)
    model_P_0gradient = NN(50,3,1,10)
    model_P_2gradient = NN(20, 3, 1, 5)
    model_P_tgradient = NN(20, 3, 1, 5)
    model_Q = NN(50, 3, 1, 10)
    model_T = NN(50, 3, 1, 8)
    ...
    text_z = torch.randn(text_x.shape[0],text_x.shape[1],1).unsqueeze(-1)
