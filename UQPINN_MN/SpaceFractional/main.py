...
import scipy.io
import csv
from SpaceFractional_UQPINN import SpaceFractional_UQPINN
...
def main():
    ...
    for i in range(run_times):
        ...
        SpaceFractional_error_evermoment, SpaceFractional_error,Inference_time, value_4, value_8= SpaceFractional_UQPINN(Nf_dataset, Nu_dataset,text_x,text_t,text_y)
    ...
    dataframe.to_csv("./Results/UQPINN_MN_SpaceFractional_noise10_alpha18_evermoment_loss.csv", index=False,header=False, sep=',')
    ...
    # dataframe.to_csv("./Results/UQPINN_MN_SpaceFractional_noise10_alpha18_value_4_approximate.csv", index=False, header=False, sep=',')
    #
    ...
    # dataframe.to_csv("./Results/UQPINN_MN_SpaceFractional_noise10_alpha18_value_8_approximate.csv", index=False, header=False, sep=',')
    #
    # dataframe = pd.DataFrame({'x': [_value_4_exact[i][0] for i in range(N_text)], 'value': [_value_4_exact[i][1] for i in range(N_text)]})
    # dataframe.to_csv("./Results/UQPINN_MN_SpaceFractional_noise10_alpha18_value_4_exact.csv", index=False, header=False, sep=',')
    #
    # dataframe = pd.DataFrame({'x': [_value_8_exact[i][0] for i in range(N_text)], 'loss': [_value_8_exact[i][1] for i in range(N_text)]})
    # dataframe.to_csv("./Results/UQPINN_MN_SpaceFractional_noise10_alpha18_value_8_exact.csv", index=False, header=False, sep=',')

    with open('./Results/UQPINN_MN_SpaceFractional_loss_noise10_alpha18.txt', 'w') as f:  # 设置文件对象
