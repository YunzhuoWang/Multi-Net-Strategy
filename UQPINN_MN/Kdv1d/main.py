...
def main():
    ...
    dataframe.to_csv("./Results/UQPINN_MN_Kdv1d_noise10_evermoment_loss.csv", index=False,header=False, sep=',')

    # dataframe = pd.DataFrame({'x': [_value_4_approximate[i][0] for i in range(256)], 'value': [_value_4_approximate[i][1] for i in range(256)]})
    # dataframe.to_csv("./Results/UQPINN_MN_Kdv1d_noise10_value_4_approximate.csv", index=False, header=False, sep=',')
    #
    # dataframe = pd.DataFrame({'x': [_value_8_approximate[i][0] for i in range(256)], 'loss': [_value_8_approximate[i][1] for i in range(256)]})
    # dataframe.to_csv("./Results/UQPINN_MN_Kdv1d_noise10_value_8_approximate.csv", index=False, header=False, sep=',')
    #
    # dataframe = pd.DataFrame({'x': [_value_4_exact[i][0] for i in range(256)], 'value': [_value_4_exact[i][1] for i in range(256)]})
    # dataframe.to_csv("./Results/UQPINN_MN_Kdv1d_noise10_value_4_exact.csv", index=False, header=False, sep=',')
    #
    # dataframe = pd.DataFrame({'x': [_value_8_exact[i][0] for i in range(256)], 'loss': [_value_8_exact[i][1] for i in range(256)]})
    # dataframe.to_csv("./Results/UQPINN_MN_Kdv1d_noise10_value_8_exact.csv", index=False, header=False, sep=',')

    with open('./Results/UQPINN_MN_Kdv1d_loss_noise10.txt', 'w') as f:  # 设置文件对象
