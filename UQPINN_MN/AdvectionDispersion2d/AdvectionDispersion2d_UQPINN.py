...
def get_r (model_P_tgradient,model_P_0gradient,model_P_1gradient,model_P_2gradient,inp_t_Nf, inp_x_Nf, inp_y_Nf, inp_z_Nf,device,Jacobian_X,Jacobian_Y,Jacobian_T):

    u_x = model_P_1gradient(torch.cat([inp_t_Nf, inp_x_Nf, inp_y_Nf, inp_z_Nf], dim=1))[:,0].unsqueeze(-1)

    u_y = model_P_1gradient(torch.cat([inp_t_Nf, inp_x_Nf, inp_y_Nf, inp_z_Nf], dim=1))[:,1].unsqueeze(-1)

    u_xx = model_P_2gradient(torch.cat([inp_t_Nf, inp_x_Nf, inp_y_Nf, inp_z_Nf], dim=1))[:,0].unsqueeze(-1)

    u_yy = model_P_2gradient(torch.cat([inp_t_Nf, inp_x_Nf, inp_y_Nf, inp_z_Nf], dim=1))[:,1].unsqueeze(-1)

    u_t = model_P_tgradient(torch.cat([inp_t_Nf, inp_x_Nf, inp_y_Nf, inp_z_Nf], dim=1))

    r = Jacobian_T*u_t-0.02*(Jacobian_X*Jacobian_X*u_xx+Jacobian_Y*Jacobian_Y*u_yy)+Jacobian_X*math.cos(math.pi/8)*u_x+Jacobian_Y*math.sin(math.pi/8)*u_y

    return r,u_t


def compute_discriminator_loss(model_P,model_Q,model_T,inp_x_Nu,inp_y_Nu , inp_t_Nu, value_Nu, inp_z_Nu):

    value_Nu_pred = model_P(torch.cat([inp_t_Nu, inp_x_Nu, inp_y_Nu, inp_z_Nu], dim = 1))

    T_real = model_T(torch.cat([inp_t_Nu, inp_x_Nu, inp_y_Nu, value_Nu], dim = 1))
    T_fake = model_T(torch.cat([inp_t_Nu, inp_x_Nu, inp_y_Nu, value_Nu_pred], dim = 1))

    T_real = torch.sigmoid(T_real)
    T_fake = torch.sigmoid(T_fake)

    T_loss = -torch.mean(torch.log(1.0 - T_real+ 1e-10 ) + \
                             torch.log(T_fake+ 1e-10 ))

    return T_loss



def compute_generator_loss(model_P,model_Q,model_T,inp_x_Nu,inp_y_Nu,inp_t_Nu,value_Nu,value_Nu_pred,inp_x_Nf,inp_y_Nf,inp_t_Nf,
                                                                         Residual,inp_z_Nu,inp_z_Nf,lam,beta,u_t_grad,u_t):

    inp_z_Nu_pred = model_Q(torch.cat([inp_t_Nu,inp_x_Nu,inp_y_Nu,value_Nu_pred], dim = 1))
    inp_z_Nf_pred = model_Q(torch.cat([inp_t_Nf,inp_x_Nf,inp_y_Nf,Residual], dim = 1))

    value_pred = model_P(torch.cat([inp_t_Nu,inp_x_Nu,inp_y_Nu,inp_z_Nu], dim = 1))
    T_pred = model_T(torch.cat([inp_t_Nu, inp_x_Nu,inp_y_Nu, value_pred], dim = 1))
    ...
    loss_adjustment = torch.mean(torch.pow(u_t_grad-u_t,2))

    loss = KL + (1.0 - lam) * log_q + beta * (loss_f+loss_adjustment)
...
class AVEMSE(nn.Module):
    ...
    def forward(self, output_Nu,Nu,u,u_t,u_x,u_xx,u_y,u_yy):
        loss_b = torch.sum(torch.pow(output_Nu-Nu,2))/output_Nu.size()[0]
        loss_f = torch.sum(torch.pow(u_t-0.02*(u_xx+u_yy)+math.cos(math.pi/8)*u_x+math.sin(math.pi/8)*u_y,2))/u_t.size()[0]
        loss = loss_b+loss_f
        return loss
...
def train(model_P_tgradient,model_P_0gradient,model_P_1gradient,model_P_2gradient,model_Q,model_T,Nf_dataset,Nu_dataset,loss_fn,device,num_epochs,optimizer_G,optimizer_T,text_x,text_t,text_y,text_z,lam = 1.5, beta = 1.,k1=1,k2=5):
    inp_z_text = Variable(text_z.flatten(), requires_grad=False).unsqueeze(-1).to(device)
    ...
    for epoch_idx in range(num_epochs):
        ...
        for batch_Nf,batch_Nu in zip(data_loader_Nf,data_loader_Nu):
            ...
            inp_z_Nf = Variable(batch_Nf[:, 3], requires_grad=True).unsqueeze(-1).to(device)
            ...
            value_Nu= Variable(batch_Nu[:,  3], requires_grad=True).unsqueeze(-1).to(device)
            inp_z_Nu = Variable(batch_Nu[:, 4], requires_grad=True).unsqueeze(-1).to(device)
            inp_x_Nf_mean,inp_x_Nf_std = torch.mean(inp_x_Nf),torch.std(inp_x_Nf)
            inp_y_Nf_mean,inp_y_Nf_std = torch.mean(inp_y_Nf),torch.std(inp_y_Nf)
            ...
            inp_y_Nf = (inp_y_Nf-inp_y_Nf_mean)/inp_y_Nf_std
            inp_y_Nu = (inp_y_Nu-inp_y_Nf_mean)/inp_y_Nf_std
            inp_t_Nf = (inp_t_Nf-inp_t_Nf_mean)/inp_t_Nf_std
            inp_t_Nu = (inp_t_Nu-inp_t_Nf_mean)/inp_t_Nf_std

            Jacobian_X = 1 / inp_x_Nf_std
            Jacobian_Y = 1 / inp_x_Nf_std
            ...
            value_Nu_pred = model_P_0gradient (torch.cat([inp_t_Nu,inp_x_Nu,inp_y_Nu,inp_z_Nu], dim = 1)).to(device)

            value_Nf_pred = model_P_0gradient (torch.cat([inp_t_Nf,inp_x_Nf,inp_y_Nf,inp_z_Nf], dim = 1)).to(device)

            u_t_grad = autograd.grad(outputs=value_Nf_pred, inputs=inp_t_Nf,
                                grad_outputs=torch.ones(value_Nf_pred.size()).to(device),
                                create_graph=True,
                                )[0]

            Residual,u_t = get_r(model_P_tgradient,model_P_0gradient,model_P_1gradient,model_P_2gradient,inp_t_Nf, inp_x_Nf, inp_y_Nf, inp_z_Nf,device,Jacobian_X,Jacobian_Y,Jacobian_T)

            G_loss, KL_loss, recon_loss, PDE_loss=compute_generator_loss(model_P_0gradient,model_Q,model_T,inp_x_Nu,inp_y_Nu,inp_t_Nu,value_Nu,value_Nu_pred,inp_x_Nf,inp_y_Nf,inp_t_Nf,
                                                                         Residual,inp_z_Nu,inp_z_Nf,lam,beta,u_t_grad,u_t)

            T_loss = compute_discriminator_loss(model_P_0gradient,model_Q,model_T,inp_x_Nu,inp_y_Nu , inp_t_Nu, value_Nu, inp_z_Nu)
    ...
    with torch.no_grad():
        ...
        u_text = model_P_0gradient(torch.cat([inp_t_text, inp_x_text, inp_y_text,inp_z_text], dim=1)).to(device)
        Inference_time = time.time()-Inference_time
        loss_text = torch.sum(torch.pow(u_text-value_text,2))/u_text.size()[0]
        print(['training_loss',PDE_loss,'text_loss:{:.7f}'.format(loss_text)])
...
def AdvectionDispersion2d_UQPINN(Nf_dataset, Nu_dataset,text_x,text_t,text_y,num_epochs=100):
    ...
    model_P_tgradient = NN(20, 4, 1, 10)
    model_P_0gradient = NN(30, 4, 1, 15)
    model_P_1gradient = NN(20, 4, 2, 10)
    model_P_2gradient = NN(20, 4, 2, 10)
    model_Q = NN(30, 4, 1, 15)
    model_T = NN(30, 4, 1, 15)
    ...
    text_z = torch.randn((text_x.shape[0],text_x.shape[1])).unsqueeze(-1)
